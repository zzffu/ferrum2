...
`4468f75`的run `30704646072/1`已通过六项独立gate与throughput，完成10k/180
stable samples后因server RSS window 2由`2182832`升至`2389976` median-twice KiB
失败；client RSS不变。Local exact `7c19e80`已在WSL2完整通过10k/180 samples/6
RSS windows/drain，但本机WSL2仍仅作test-code diagnostic，不能替代hosted资格。
