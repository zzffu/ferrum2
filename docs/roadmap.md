...
| 2026-08-08 | M14-T05 config-CLI latch lease | T05 additionally owns the existing `tests/m0-harness/tests/config_cli.rs` row only to replace the superseded server schema-v2 startup-latch expectation while retaining the client latch until T07 | Exact T05 Quick reaches real server typed-route composition and therefore returns `startup.bind` on the deliberately occupied endpoint；the unchanged T03 row still requires both roles to stop at `startup.protocol` and is now the sole failure | Make the existing table role-specific：client remains `startup.protocol`，server requires `startup.bind`；no product、helper、harness、config acceptance、workflow policy or remote expansion，and ticket base remains `8d138300…` |
