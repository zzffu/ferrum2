...
| DEC-017 | proposed M0 narrow amendment | external四案先比较双向各16386-byte distinct payload，再证明ordered FIN/EOF；peer FIN后新reverse drain继续由同一SHA的M0-E2E-001/M0-LIFE-003独立blocking；pin/wire/product不变 | `ADR-0014`、`SPEC-0001`、`TEST-0001`、M0-T08 |
...
| 2026-07-27 | M0 external half-close evidence amendment | proposed：保留sing-box 1.13.14 pin与四项hard gate，把external evidence细化为双向完整bytes后ordered FIN/EOF；ferrum FIN后reverse drain仍由local/runtime同SHA硬门证明 | exact diagnosis隔离出sing-box 1.13.14在peer FIN后关闭leg的第三方限制；不改变wire/product/API/pin/remote范围 | ADR-0014 Proposed；Product triage PASS；Architect/QA final pending |
