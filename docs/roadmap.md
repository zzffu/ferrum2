...
`13b6d681…` 已通过 Architect/QA `PASS` 和 warning-free package/workspace Clippy，T02 remains done，
M16-T03 ready / current frontier；仍不从该单一 build 推断跨版本兼容，也不声称该 repair 已集成、
推送或 hosted-PASS。
...
| M16 | M15 closed | Windows IPv4 compatible capture/Wintun DNS与route-selectable client direct egress |
...
M16把两个待办合成一个bounded outcome：client outbound成为closed Direct/Shadowsocks sum，现有plan/
route/selector决定TUN或SOCKS TCP/UDP是否走SIP022；Windows auto-route只创建IPv4 include-minus-
exclude的Wintun `/1` capture rows，并用capture前IPv4 physical policy pin每个reachable IPv4 proxy/
direct/DNS socket。IPv4 direct target使用一个冻结的physical default interface；Windows TUN-selected
direct IPv6在auto-route on/off都于socket前fail closed。Auto-DNS只steer owned Wintun IPv4 DNS并exact
hijack synthetic IPv4 port 53，不承诺WFP/anti-leak。M15 manual-route IPv6、SIP022 IPv6与non-managed/
SOCKS direct IPv6保持。T01先以当前exact VM/checkpoint真实IPv4 probe冻结route values并证明hard-kill
zero residue；失败即replan，不从单build结果推断Windows跨版本兼容。
...
- **Objective:** Add client `[[outbounds]] type = "direct"` as one terminal singleton egress plan and let
  ...
  Wintun-only DNS steering with exact rollback。Preserve M15 manual-route IPv6 and existing IPv6 regressions。
- **Baselines:** qualified M15 product `7ba6268ffa3c5ecc7ba2b91e3ebcae8f596ecbb9` / tree
  `72a3cfb5c881a35b1416cbf9ffea593973cc3570`；planning
  `fcef80dcc7e62bbca63ffbf7832df369dd418abd` / tree
  `ddeb8afba64729f08a334d9c8a56e17e92c1d224` / parent `ba637e08287cbb623d4b604123f27e9e8b2df537`。
- **Contracts/research:** ADR-0035/0036、SPEC/TEST-0017、
  `docs/research/M16-windows-auto-route-dns-direct-reference.md`。Terminology separates one-hop proxy、client
  direct、route exclusion、physical underlay binding and DNS steering。
- **Ownership:** `EgressPlanSnapshot` and core routing remain opaque；private client egress dispatches the
  Direct/Shadowsocks sum from one closed binary-private SOCKS/TUN/DNS request origin plus selected plan/original
  target；callers remain kind-agnostic and socket-free。Existing `ferrum2-wintun` owner deepens for route/DNS/
  binding/notification lifetime；no process-wide TUN-presence policy、new crate、dispatcher or public seam。
- **Underlay limit:** while auto-route is active，reachable physical first hops must resolve to IPv4；fixed
  proxy/DNS endpoints use endpoint-specific capture-before binding，and dynamic IPv4 direct uses one frozen
  physical default interface。IPv6 concrete proxy/direct-no-detour DNS physical endpoints fail before mutation，
  while a logical IPv6 DNS bootstrap behind an IPv4 proxy first hop remains allowed。Windows TUN-origin direct
  uses the IPv4 default binder and rejects IPv6 pre-socket for both auto-route states；SOCKS origin using the
...
- **Tickets:** planning/control `a9619ef…` → single-VM amendment `a8205c2…` → accepted Markdown-only IPv4 scope
  ...
  `13b6d681…` locally repaired/done → T03 direct TCP/UDP ready → T04 capture/binding → T05 DNS
...
- **Footprint/performance:** baseline code/tests `29771/50323`、case/support/fixture `44574/5152/597`；forecast
  `+2250..3650 / +160..460 / +0` Rust tests plus existing qualifier growth。T02 reports integrity `PASS` and
  ...
  seam evidence with no support/fixture growth。Performance is required without a threshold/improvement claim。
...
- **Next/remote boundary:** M16-T02 remains done and M16-T03 is ready / current frontier on exact local repaired
  candidate `13b6d681…`。The first T02 non-force push integrated `5782c08…` to `master` with exact readback；
  ...
  closed，and push-event performance jobs were skipped。No hidden second failure exists。`13b6d681…` is not
  integrated、pushed or hosted-PASS。Required future non-force pushes，including performance-related pushes，
...
| IPv4 direct/proxy/DNS socket在capture后未pin而递归进入Wintun，或process-wide TUN check误拒SOCKS IPv6/auto-route-off DNS | P0 | M16-T01 | fixed/dynamic IPv4 A/B；closed engine request origin；mixed same-tag SOCKS-success/TUN-pre-socket-fail TCP/UDP table；auto-route-off IPv6 DNS omission row；无unpinned fallback |
...
| preflight被误记为PASS或IPv4 replan误删M15 IPv6保证 | P0 | M16-T01 | preflight仅作高层planning rationale；M15 `16/16`与IPv6 adapter/manual-route rows保留；M16 markers独立为IPv4 direct `1/1`、DNS `2/2`、change `3/3` |
...
| 2026-08-05 | M12-T03 ownership repair | T03 owns the existing DNS crate manifest/module root and `Cargo.lock`；a bounded server `run.rs` lease permits three isolated inherited Rust-1.88 let-chain repairs | T01 intentionally left an empty compiling crate；the manifest/root/lock are required to declare modules、consume existing workspace dependencies and keep `--locked` metadata current。Full also reproduces only the three known `collapsible_if` warnings at server lines 439/467/1397 | Root `Cargo.toml` and new dependencies remain out of scope；server repair must be a separate mechanical commit；branch/worktree starts at exact `bf26d158` with budget binding PASS；no remote or publication action |
| 2026-08-05 | M12-T03 encrypted-fixture unblock | T03 may centrally pin exact already-locked `h2 0.4.15`、`rustls 0.23.43` and `tokio-rustls 0.26.4`，enable Hickory server `https-ring` only on the DNS dev edge，and add reviewed static synthetic DER fixtures；one exclusive ferrum-owned DNS runtime contains Hickory's direct DoH spawn | Engineer exhausted two API-only attempts at exact `30352074`；two independent `gpt-5.6-sol/xhigh` read-only analyses agreed on direct Rustls/static certs/no rcgen or insecure verifier。Workspace-pinned policy was selected over package-local versions because versions live centrally；exclusive runtime closes the lifecycle gap absent from the narrower proposal | Ownership adds root `Cargo.toml`、workspace-policy test and four fixture paths；no new package identity、provider、normal Hickory server feature or product custom-CA surface；run exact graph/MSRV/policy/T03/Full gates；no remote or publication action |
| 2026-08-05 | M12-T03 feature-graph repair | Replace the invalid dev-only `hickory-server/https-ring` choice with exact-pinned internal `hickory-server/__https`；`__https` implies `__tls` and exposes the required server APIs without the optional resolver edge | Generated `https-ring` lock had 241 identities、`system-config` and duplicate socket2；manual 222 trimming was rejected by locked metadata after two repair rounds。Two new independent `gpt-5.6-sol/xhigh` analyses both selected `__https` from exact 0.26.1 feature/API evidence | Keep 222 identities and normal featureless server edge；policy pins `__https/__tls` and forbids `hickory-server -> hickory-resolver`、system-config and new providers；internal-feature risk is bounded by exact version/checksum and must be re-reviewed on upgrade |
| 2026-08-05 | M12-T03 integration | exact `e1a9ae37` integrates candidate `10b7c31f` with bounded UDP/TCP/DoT/DoH、immutable plan snapshots、owned admission/deadline、closed errors and an exclusive awaited DNS runtime；T03 done，T04 active | Initial review found cancellation/admission、Drop/join、detour ownership/evidence and package-fixture gaps。One bounded repair closed QA findings；remaining pre-first-poll counter and blocking-Drop findings each used the required two independent `gpt-5.6-sol/xhigh` read-only diagnoses。Final owned guard、closed TaskSet and `TaggedResolverOwner` solution closed all IDs；Architect/QA both `PASS` | Integration tagged `2/2`、lifecycle `7/7`、DNS `14/14`、policy `21/21`、Rust 1.88、Full、ignored lifecycle `1/1` `130.71s` and docs PASS；footprint integrity PASS and numeric `+2326/0/0`、code `+813`、ratio `2.026931` explicitly ACCEPT；no push/hosted/release/publication |
| 2026-08-05 | M12-T04 ownership completion | T04 additionally owns the existing DNS module root、client composition manifest/root、lock and exact workspace-policy row needed to expose `proxy.rs` and add the already-declared `ferrum2-dns` workspace edge | The planned four-file list could not compile a new library module or client dependency without these mechanical integration paths；the amendment is made before assigning the ticket writer | No new dependency identity or scope；T04 remains one writer and must update only the exact client/DNS dependency edge plus existing policy assertion |
| 2026-08-05 | M12-T04 Hickory framing/test boundary | Public TCP framing must use exact 0.26.1 `hickory_resolver::net` re-export；client may add exact workspace `hickory-proto` only as a dev edge for typed request/response fixtures and assertions | Local exact source confirms `hickory-resolver` already re-exports `hickory-net`，while the failed raw response oracle retained EDNS OPT in an invalid section；typed Hickory construction/parsing fixes evidence without product behavior changes | No root/DNS-manifest edge、new identity/provider/API or handwritten DNS TCP framing；owned client manifest/lock/policy paths cover the only dependency delta |
| 2026-08-05 | M12-T04 response-seam ownership | T04 additionally owns existing `resolver.rs` and `runtime_owner.rs` to extend the one T03 command path with a Hickory response-preserving single-question operation；the existing address lookup remains for T05 server composition | Exact T03 seam returns `Lookup` on success but maps Hickory negative responses to closed NXDOMAIN/NODATA errors；Hickory `NoRecords` omits full negative additional/EDNS，so `proxy.rs` alone cannot preserve the selected response required by M12-MUST-05 | One writer、one existing command/runtime/provider path；no raw DNS data plane、second parser/framer、new package/provider or change to application address-lookup behavior |
| 2026-08-05 | M12-T04 bounded Hickory Stream edge | T04 additionally owns root and DNS manifests for exact workspace `futures-util = 0.3.33`、no-default、`std` direct use；the DNS crate uses only `StreamExt` to poll Hickory's public inbound `net::tcp::TcpStream` inside ferrum2's bounded accept loop | Hickory's public TCP framer implements `futures_util::Stream` but does not re-export `StreamExt`；its stock server listener owns an unbounded accept path，so neither transitive naming nor the stock loop can satisfy bounded multi-query connections | Version/identity and `std` are already locked/resolved through Hickory；no package identity、feature、provider、wire implementation or public surface expansion，and workspace policy must prove the unchanged graph |
| 2026-08-05 | M12-T04 integration | exact `7d2cadab` integrates candidate `289c5842` with bounded client UDP/TCP DNS proxy、mandatory-final routing、real direct/Shadowsocks detours、selector snapshots、shared UDP ownership and awaited resolver cleanup；T04 done，T05 active | Initial Architect/QA found cancellable owner handoff、missing real matrix、queue accounting、maximum valid QNAME and protocol/lifecycle evidence gaps。Exactly one bounded repair closed every ID；targeted Architect/QA both `PASS`，so the post-review dual-agent path was not triggered | Integration core/client/DNS、policy `21/21`、Full、Rust 1.88、ignored lifecycle `1/1` `130.37s`、docs and locked metadata PASS；footprint integrity PASS and numeric `+2260/0/0`、code `+433`、ratio `2.104062` explicitly ACCEPT；no push/hosted/release/publication |
| 2026-08-05 | M12-T05 ownership completion | T05 additionally owns the existing server manifest、lock row and workspace-policy assertion needed to add the already-declared local `ferrum2-dns` direct edge | The server cannot import T03's public resolver/owner API through a transitive dependency；adding the direct local edge updates both the server lock dependency row and its exact policy set | No root dependency、new package identity/feature/provider or `main.rs` edit；`run.rs` declares the sibling egress module and the clean T05 branch advances to this corrected contract before product work |
| 2026-08-05 | M12-T05 provider/control ownership | T05 additionally owns the existing external qualification support and contract test；hosted CoreDNS/BIND provisioning may update `m0.yml` only in an isolated single-parent control-only commit before Rust/product work | The owned orchestration module contains no process/provider implementation，and duplicating it would violate the one-harness contract。The hosted interop job currently provisions only the two SIP022 references，so it cannot require the new DNS result without a narrow control change | Reuse `external_support` and the existing interop job；no second harness、trigger/performance/manual-dispatch/unrelated-job change。Later product commits inherit the control blob unchanged and ticket footprint must validate the complete range |
| 2026-08-05 | M12-T05 encrypted interop trust boundary | Implement ADR-0031's selected-profile trust injection as one default-off internal `ferrum2-dns/__interop-test-root` feature，embedding only the reviewed M12 CA for isolated CoreDNS DoT/DoH client/server qualification builds | Normal WebPKI cannot trust a loopback private-CA CoreDNS，while isolated CI cannot possess a public-root private key；dropping encrypted positives or adding a product custom-CA field would each violate the approved contract | Keep chain/time/name verification；no config/CLI/environment/runtime root input or insecure verifier。Amend the sole pre-product control commit to enable the feature only in an isolated target and delete it after use；normal/default/release artifacts remain WebPKI-only |
| 2026-08-05 | M12-T05 hosted pre-review test lease | T05 receives one bounded lease on the existing client `run.rs` test module to repair only `dns_proxy_first_match_direct_and_detoured_transports` and an already-shared helper if required | Exact candidate `bd2960cd` passed local Full and hosted interop/platform/footprint，but both Linux quality and MSRV independently failed because the test retransmits UDP queries as a startup probe against one-shot upstream fixtures；this is pre-review CI repair，not the formal review repair round | Remove the duplicate-query race at the test readiness boundary；no client product/config/public-surface change。Engineer validates repeated focused runs plus Full/MSRV，then a fast-forward push triggers a fresh hosted run |
...
| 2026-08-07 | M13-T03 workspace-policy lease | T03 additionally owns existing `tests/m0-harness/tests/workspace_policy.rs` only to replace the superseded DNS `ferrum2-config` expected normal edge with approved `ferrum2-core` | Quick exact test failed twice on one stale expected map；ADR/SPEC、architecture guard、manifest、metadata and lock all independently require/prove only core，and full-repo search found no sibling hardcode | One-line existing policy update plus exact focused gate；no threshold、feature/provider/package、product、workflow-policy or remote change；ticket base remains fixed and later integration preserves this control lease |
| 2026-08-07 | M13-T03 integration | DNS owns its closed runtime spec and depends internally only on core；both binaries convert validated DTOs before side effects，and DNS detours/TC use the originating core snapshot allocation；T03 done、T04 ready | Initial Architect/QA `BLOCK` on formatting-specific duplicate-snapshot guard、origin-allocation mutation evidence and RED auditability；one bounded four-file evidence repair `dd53259` plus authentic retained RED record closed all IDs，targeted reviews `PASS` | Merge integration `768c750a` passed full T03 and Quick；footprint integrity PASS、case/support/fixture `+219/0/0`，review-sized existing binary tests accepted；no new helper/fixture/harness/dependency/provider，no remote action |
...
| 2026-08-08 | M14-T05 control-evidence leases | T05 additionally owns only the existing schema-v2 latch row in `config_cli.rs` and server dependency expected set in `workspace_policy.rs` | Exact Quick first proves real server typed-route composition now reaches the occupied endpoint and returns `startup.bind` while the client latch remains；after that correction，the sole next failure is the stale server lock-dependency set omitting its required normal `ferrum2-sniff` edge | Make the existing CLI table role-specific and add only `ferrum2-sniff` to the server expected set；no helper、harness、feature、identity、config acceptance、workflow policy or remote expansion，and ticket base remains `8d138300…` |
| 2026-08-08 | M14-T05 formal-review repair lease | Initial Architect/QA both `BLOCK` exact `da0d9a21` because the mandatory closed sniff observation tuple is absent；Architect also found cancellation fast paths、prefix allocation/accounting、UDP reject ordering and mutation guards incomplete | The existing observability owner is the only correct home for fixed stage/outcome/protocol values，and the runtime owner guard is the existing buffer-lifetime seam；duplicating either in server composition would violate the one-interface contract | Lease only the existing observability source/two contract tests and runtime `owner.rs` in addition to already-owned T05 paths；one bounded repair must reuse current server/runtime tables and architecture scanner，with no ADR、new harness、dependency、fixture or remote action |
| 2026-08-08 | M14-T05 integration / escalation closure | Exact `4c65bb02` integrates authenticated server TCP/UDP ordered routing、bounded sniff、closed telemetry、terminal no-fallback and one unified protocol-session ceiling；T05 done、T06 ready | Initial Architect/QA `BLOCK` drove one bounded repair；targeted re-review still found real `limit` unreachable、direct+reject state exceeding one ceiling and no combined selected-open-failure witness。Two required independent `gpt-5.6-sol/xhigh` read-only analyses independently converged on the same existing-seam correction，then final Architect/QA both `PASS` with all IDs closed | Integration focused and Quick PASS；Rust 1.88 check/build/test PASS on accepted product；footprint integrity/category/ratio PASS and advisory `+1508/0/0` accepted for distinct security/composition rows with no helper/support/fixture/new harness；first integration CLI attempt used a stale unbuilt bin and passed `6/6` after required bin build；no remote action |
| 2026-08-08 | M14-T06 unknown-qtype policy lease | T06 additionally owns only `ferrum2-config`'s existing client DNS policy model method and its authoritative contract table | Hickory 0.26.1 can represent valid wire qtypes outside M14's closed matcher set，but T03's selector accepts only `DnsQueryType`；mapping unknown to `ANY` spuriously matches，skipping policy loses qtype-less rules，and returning SERVFAIL regresses M12's accepted record-type behavior | Let the existing ordered DNS program receive absent protocol metadata for unknown qtypes so qtype rules do not match while qtype-less rules/final remain ordered；no new qtype spelling、parser、evaluator、config acceptance、dependency、helper or T07 ordinary-hijack composition |
| 2026-08-09 | M14-T06 post-review typed-fixture lease | Targeted QA retained the server root policy-handoff mutation and found the repaired live test's fixed qtype offsets/header-bit edits to be a copied DNS codec。Per execute instructions，two independent `gpt-5.6-sol/xhigh` read-only analyses both confirmed the findings and selected the same repair | No reusable exported Hickory fixture exists，`DnsProxy::answer` would recursively require another resolver，and moving the row to DNS tests would lose private binary composition。The already exact workspace `hickory-proto 0.26.1` is the smallest typed fixture boundary | Lease only server `Cargo.toml`、its lock dependency row and existing workspace-policy test；add a no-default `std` dev edge，replace manual bytes inline with Hickory public types，and guard the exact `ServerDnsState::new(route, policy)` handoff plus test-module codec escapes。No new package/feature/product edge/helper/fixture/harness or remote action |
...
| 2026-08-11 | M16-T02 post-push Clippy repair | First T02 non-force push moved `master` `5630cf4…` → `5782c08…` with exact readback；automatic `31443577764/1` is preserved failed and was not rerun | MSRV、interop、footprint、three platforms and Windows TUN E2E passed；quality failed only at deterministic `too_many_arguments`、`map_clone` and `redundant_closure` lints，qualification failed closed，push-event performance jobs skipped，no hidden second failure | One-file mechanical `13b6d681…`（tree `c8cf7df…`，parent `5782c08…`，`+3/-2`）passes package/workspace Clippy、focused `4/4`、full config `33`、architecture `19`、workspace/fmt/hook/diff and Architect/QA `PASS` with zero findings；integrity PASS，numeric `30096/51191`、`1.700924`、`+756/0/0` advisory；repair not integrated/pushed/hosted-PASS，no rerun/dispatch/force-push/PR/tag/package/release/publication |
