...
terminates，every setup/cleanup ordinal is mutation-tested，and fresh Windows 10/11 full、100-cycle and
hard-kill profiles prove zero owned residue without a new harness or widened security claim。
...
- [ ] Fresh Windows 10 and Windows 11 full profiles cover direct/proxy IPv4/IPv6 TCP/UDP，system DNS UDP/TCP，
      one real route mutation、one real interface mutation and real IPv4 plus IPv6 unicast-address mutations，
      graceful/forced stop and sentinels on one exact candidate。Every network-change row proves callback
      observation、new-admission rejection、capture/DNS removal、supervised termination and residue cleanup。
- [ ] Each guest completes 100 cycles with zero OS and process-private owner counters。Separate external
      `TerminateProcess` rows prove only process absence and zero adapter/address/route/DNS residue before
      controller remediation/checkpoint restore and emit the exact TEST-0017 `cases=3/3` hard-kill marker。
...
Run privileged commands independently in each approved guest；the host performs only Hyper-V control and
read-only residue audit。
