...
- [ ] T01 public config/plan、four T02/T03 focused composition rows and both T04 real-process rows pass on
      the accepted exact SHA with no private mutation or duplicated encoder/harness。
- [ ] Serial Full、Rust 1.85、100+ lifecycle、three native targets and existing TCP/UDP `12/12` each plus
      cleanup pass without SHA/run/attempt splicing。
- [ ] Schema 3 integrity passes；numeric footprint signals are explicitly accepted、reduced or reforecast
...
Run `TEST-0012` T05 focused reruns and serial integration gate exactly as recorded。Remote commands remain
unrun until separately authorized。
