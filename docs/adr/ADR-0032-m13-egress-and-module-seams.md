...
`RouteTable::final_plan_snapshot` are the client and DNS-detour data-plane interface。The existing
borrowed `snapshot`、`select_plan` and `final_plan` remain compatible views；M13 does not remove them。A
direct plan remains one hop，a chain remains `2..=8` ordered concrete hops，and a selected snapshot is
