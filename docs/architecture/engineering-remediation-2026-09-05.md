...
| M2e / RD-04 | DNS共享SystemResolver实际OS工作owner，通过net traits注入全部runtime/RuleSet/binary路径；queued/running/finished-unjoined共用permit；删除隐式system默认构造及lookup_host；两端outer scope在所有用户结束后join | DNS/runtime/ruleset全包、8个native-owner小规模测试、严格lint通过；client no-run、server56、两端build、m0 materialize2通过。末次review将无DNS cap从bootstrap1改为runtime连接cohort（上限4096）、adapter timeout保留connect_timeout；此修订整合门禁被后续network/UDP接口迁移阻塞，待统一冻结重跑。OS调用无法强制取消，无性能结论 |
| M4d / C1 | 32个metric维度单声明生成enum/labels/ALL/index；删独立枚举表、调用点discriminant算术及non_exhaustive；补Dropped/Tun合法grid | observability25及strictlint/fmt通过；UDP完整56tuple及Dropped/Tun精确series测试。增加196个预分配counter cells（4connection+8UDP+184failure），未触及不渲染；MSVC布局按24B/label-counter tuple推导约4704B，非实测RSS。首轮重复Transport import失败已修正 |
| M3c2 / D5 completion | supervisor在构造run前保管cleanup；先报告task exit并取消其他roots，再poll独立清理到join后reap；TUN共享owner保管原JoinSet/native；ProcessResources绑定materialize前baseline及全局最后cleanup | full runtime、TUN135及两包全target严格lint/格式通过；6个owned-cleanup、4个final-resource测试含peer取消/cleanup依赖、root panic/abort/未poll、全部早退与残留检测。任意丢弃整个supervisor future仍不承诺异步join；调用者必须持续驱动到report，绝不标为成功 |
