...
a null key and the SYN's reference for the otherwise valid ACK. Each packet now retains
its own key snapshot: DATA at header observation, SYN/ACK after their key-state observation.
...
input paced at 16, 64 and 256 MiB/s, all three short captures completed; callback p99 was
22.3–26.0 microseconds and output occupied approximately 22.44 MB. These short bursts do
not establish a sustained throughput limit. A 64 MiB input paced at 512 MiB/s stopped
capture with `queue_full` after recording approximately 21.36 MB of raw input; its later
cheap no-op calls are not evidence of fast complete recording.

Absent `[rocom]`, relay selection bypasses capture construction and observation entirely.
Enabled nonmatching traffic is rejected in memory and writes no capture file. The matched
path adds copying/allocation, queue submission, base64/JSON work and storage traffic.
Idle worker shutdown took about 93 ms in this probe. Queue bounds and per-file byte caps
do not bound disk latency: writes, `sync_all` and worker join can extend process exit beyond
network shutdown grace. Prefer private local storage; `max_bytes` is not a directory-wide
disk budget. No full-proxy percentage regression, WAN or privileged TUN performance claim
is made by this probe.

Final affected-package all-target/all-feature Clippy (`-D warnings`), workspace formatting
and API documentation gates passed. The actual decoder CLI processed a finalized synthetic
capture with 513 messages, zero source-integrity errors and a complete source; exit 2 was
expected for its 512 non-business DATA payloads that failed bounded payload decoding.
Temporary probes, synthetic captures and decoded plaintext were removed after verification.
