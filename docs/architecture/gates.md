...
| G-TUN-MOCK-COMPILE | `m0.yml:quality:Compile TUN-only mock benchmark` | ordinary quality gate | `cargo check -p ferrum2-tun --example tun-benchmark --no-default-features --features benchmark --locked` | ordinary Linux; compile-only | real packet/owner implementation with no live backend enabled | wired |
| G-TUN-MOCK | local candidate controller `tun-mock-calibrate/run/validate` | explicit measurement, not a timing-sensitive PR gate | six TUN-only bounded mock-I/O recipes, Quick 36 trials or Confirm 60, independent source builds and reviewed A/A-bound A/B | no privilege or network mutation; bounded child jobs/groups | retained binaries, logs, raw trials and strict recomputed manifest | new-source calibration required |
...
| G-PY-PLATFORM | `m0.yml:quality:Test native qualification controller contract` | same | `python3 -B -m unittest discover -s tests/platform -p 'test_*.py' -v`; native diagnostics, startup rollback, cleanup readback and port handoff | ordinary Linux; PowerShell cases skip if unavailable | stdlib tests, injected Windows state and unprivileged loopback reservations; no real adapter | current named step |
...
| G-WIN-HOST-QUAL | local `run_windows_tun_qualification_host.ps1` | manual authorized Windows host; never CI | exact candidate, selected IPv4/IPv6 single stack, real Wintun create/delete, sustained concurrent TCP/backpressure/half-close, UDP/fragments, active-work reset, precise WFP epochs, forced recovery and zero residue | <900s outer / 840s worker / 600s build / 60s workload; elevated and acknowledged | closed source and family identity plus checked two-generation workload required in final `QUALIFIED` verdict | exact-source live evidence required separately per family |
| G-PS-STATIC | `m0.yml:platform / windows-msvc:Validate Windows qualification static contracts` | PR/push static contract | closed qualification source bundle, both-family PlanOnly contract, acknowledgement fail-closed behavior, family-scoped ownership and recovery readback | ordinary Windows; nonmutating | no network mutation and no live qualification claim | wired |
...
- TUN performance uses bounded mock I/O without host networking. It shares production packet and
  owner work, but does not qualify actual socket reactor or Windows behavior. Real-loopback tests
  cover readiness, write capacity and half-close. The sole privileged host qualification owns
  sustained traffic/reset witnesses, process/cleanup primitives and its 900-second supervisor.
