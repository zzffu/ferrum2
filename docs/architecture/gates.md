...
| G-WIN-HOST-QUAL | local `run_windows_tun_qualification_host.ps1` | manual authorized Windows host; never CI | exact candidate, real Wintun create/delete, sustained concurrent TCP/backpressure/half-close, UDP/fragments, active-work reset, precise WFP epochs, forced recovery and zero residue | <900s outer / 840s worker / 600s build / 60s workload; elevated and acknowledged | closed source identity and checked two-generation workload required in final `QUALIFIED` verdict | exact-source live evidence required |
