...
| WIN-02..06 | Windows platform crate | DLL identity/export/System32 rules, typed handles, immediate LastError, exact managed rollback, callback/WFP/session cleanup | injected Windows unit cohorts enumerated in `refactor-consumers.md`; live qualification runbook | Hosted tests stay behind injected operations; rename/split must update exact IDs | ordinary Linux + hosted Windows + approved Hyper-V live | pending live |
| WIN-07 | platform + TUN | Ring full is one counted drop with no retry/reset/rebuild | hosted-safe Wintun/TUN unit tests + scheduler-ring-full profile | Retain exact live counter witness | ordinary unit + approved Hyper-V | pending live |
| TUN-01..05 | TUN owner | Lightweight reset versus full rebuild, debounce/audit, transition ordering, same-logical-reset settle and exactly-once events | hosted-safe TUN library suite; network-reset/restart profiles | Default tests must remain adapter-free | ordinary Linux/Windows + approved Hyper-V live | pending live |
| TUN-06/07 | TUN data plane | Canonical packet validation and strict bounded reassembly; reviewed corpus remains separate from fuzz seeds | `reassembly-v1.hex` + provenance; deterministic smoke; four fuzz targets | Hosted execution remains pure in-memory; real TUN remains guest-only | ordinary unit + hosted smoke/fuzz + approved guest live | baseline |
| TUN-08..10 | TUN data plane | Initial-SYN admission, TCP cleanup/backpressure, UDP EIM/EIF/ADF, no live eviction, unmetered exception remains narrow | hosted-safe TUN library suite and fuzz race corpus | Live adapter evidence remains separate | ordinary unit/fuzz + approved Hyper-V | pending live |
...
| PLAT-01/02 | platform scripts | Privileged execution only in approved guest; public main interface is `Core`/`Endurance`/`Release`; candidate artifacts build once, profiles run in fixed order through fresh transactions, exact `lab_checkpoint` restores and final Off; hard-kill stays independent and cleanup failure cannot pass | M17 runbook; schema-4 identity ledger; neutral controller bundle; 28/21 runtime and 33/25 host-source closures; main staged v6/host v7/campaign v1; hard static/staged/host v4 | Live evidence intentionally unavailable in ordinary R0 | ordinary static + approved Hyper-V live | pending live |
| PERF-01..04 | qualification/controller | Performance and qualification share only neutral Lab mechanics; the 38-source performance closure hash-binds its four Lab runtime owners and contains no qualification module; producer correctness stays separate from reviewed adoption policy; calibration identities are closed, inconclusive states never pass, and raw evidence remains recoverable by digest | performance source manifest, policy/controller tests and Gate Ledger | 30-day workflow artifact is not durable provenance | manual performance only | pending retention |
