...
state machine. IPv4 copied-option normalization lives in its private
`reassembly/ipv4.rs` owner; complete-fragment behavior remains tested through reassembly.
The packet owner is already split: `packet.rs` owns target-neutral parsing and validation,
...
| LIFE-01/02/03 | runtime + bins | Prepare-before-activate, reverse rollback, admission/drain/cancel/join order, owner baseline, exactly-once reap and rebind; client startup carries required network resources as one constructed bundle | runtime `lifecycle_{transaction,root_events,accept,relay}.rs` and `shutdown.rs`; m0 `lifecycle_cycles.rs`; managed loopback restart/rebind | Cross-bin rollback matrix remains a characterization task | ordinary; every-push lifecycle stress | current |
...
| TUN-06/07 | TUN data plane | Canonical packet validation and strict bounded reassembly, including first-only IPv4 non-copy options; reviewed corpus remains separate from fuzz seeds | `reassembly-v1.hex` + provenance; reassembly option/order tests; deterministic smoke; four fuzz targets | Hosted execution remains pure in-memory; live qualification covers transport integration, not corpus replay | ordinary unit + hosted smoke/fuzz | current contracts; live evidence remains revision-bound |
| TUN-08..10 | TUN data plane | Initial-SYN admission, system TCP cleanup/backpressure, UDP EIM/EIF/ADF, no live eviction, independent TUN UDP byte admission, and deadline validation at commit/response consumption | hosted-safe TUN library suite including direct single-step deadline cases and fuzz race corpus; live TCP/UDP host probe | Detailed policy evidence remains hosted; the live gate proves basic real-adapter transport | ordinary unit/fuzz + explicit host qualification | current contracts; live evidence is revision-bound |
...
| ID | Owner | Observable contract | Existing test/evidence | Gap / affected PRs | Gate | Last verified |
|---|---|---|---|---|---|---|
| DNS-01..04 | `ferrum2-dns` | Each tagged query retains its permit/deadline and same-server TC upgrade; strict encrypted DNS and no system fallback; explicit evaluate/response matching never queries an unselected upstream; cache admission preserves full response-policy semantics and CNAME lifetime | DNS policy/proxy/tagged/resource/application tests, including oversized address sets and alias negative expiry; [explicit response policy](dns-response-policy-design.md) and loopback client query-count evidence | None known | ordinary DNS interop-root | current |
| RS-01..05 | `ferrum2-ruleset` + shared rule interval policy | Remote HTTPS only, verified cache, checked deadlines, atomic initial snapshot, monotonic refresh and redaction; disk-ahead/live-old content converges after successful 304; refresh interval is 1 second through 365 days | ruleset loader/HTTPS tests, concurrent 304 reconciliation and configuration/source interval bounds; SRS and shared DNS-TLS fixtures | Transaction-failure injection and later reconciliation are separate regression surfaces | ordinary ruleset + config | current |
