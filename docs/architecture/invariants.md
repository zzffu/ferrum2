...
| ID | Owner | Observable contract | Existing test/evidence | Gap / affected PRs | Gate | Last verified |
|---|---|---|---|---|---|---|
| DNS-01..04 | `ferrum2-dns` | One server/permit/deadline, same-server TC upgrade, strict encrypted DNS, no system fallback, complete nonblocking cleanup; cache admission preserves full response-policy semantics and CNAME lifetime | DNS policy/proxy/tagged/resource/application tests, including oversized address sets and alias negative expiry | None known | ordinary DNS interop-root | current |
