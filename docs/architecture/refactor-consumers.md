...
| fuzz owner-impact paths | `workspace_policy/architecture.toml:[fuzz_impact]` | refactor review and trigger-impact audit | include full TUN and Windows source trees, all platform controller sources, fuzz workflow/workspace, the exact transitive local path dependencies, root manifest/lock and vendor; the independently required workflow itself runs on every PR/protected push |
...
| fuzz `Cargo.lock` | standalone fuzz workspace | deterministic/libFuzzer build and guest smoke | `--locked`; nightly and local vendor patch agree |
