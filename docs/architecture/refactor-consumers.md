...
| Windows TUN correctness entrypoint | `tests/platform/run_windows_tun_qualification_host.ps1` | operator runbook, static contract, worker supervisor, final verdict | PlanOnly/one acknowledged run select IPv4 or IPv6 (default IPv4); RecoveryOnly derives family from ledger; no suite, profile, guest, or alternate entrypoint |
...
| fixed host qualification plan | `Ferrum2.Qualification.Host` | static PlanOnly readback and live worker | exact eight checks; 900s outer, 840s worker, 600s build; one exact candidate build; run-owned RFC 2544 `/32` or ULA `/128` explicit routes; only owned TUN connected `/30` or `/126` |
| qualification evidence schema family | schema-v1 plan, build, runtime, worker, cleanup, and final documents | operator and final supervisor | candidate/source/address-family identities agree; all checks pass; final status is `QUALIFIED`; cleanup counts are zero; elapsed is below 900s |
