...
| M4 | 可复现性能基线、资源门与 v0 preview integrated qualification | planned |
...
diagnostic throughput与bounded 10k-idle profile；本机WSL2仅用于test-code
diagnostic。下一入口是`mode=execute milestone=M4`；plan不自动执行、推送或
发布。
