...
#[test]
fn extended_ack_keeps_its_observed_key_across_interleaved_syn() {
    let mut upload = Stream::default();
    let mut download = Stream::default();
    let mut keys = KeyState::default();
    let mut output = Vec::new();
    ...
    let syn = packet(wire::SYN, &[2, 3], &[]);
    upload
        .feed(&syn, &mut keys, &context, &mut output, &mut report)
        .unwrap();
    let mut extension = vec![2, 16];
    extension.extend_from_slice(&[7; 16]);
    extension.push(0);
    ...
    context.event = 2;
    download
        .feed(&ack[..39], &mut keys, &context, &mut output, &mut report)
        .unwrap();
    context.direction = Direction::Upload;
    context.event = 3;
    upload
    ...
    context.direction = Direction::Upload;
    context.event = 5;
    let subsequent = packet(
        wire::DATA,
        &[],
        &encrypted(&[0, 0, 0, 0, 0, 0, 0, 1, 8, 42]),
    );
    upload
        .feed(&subsequent, &mut keys, &context, &mut output, &mut report)
        .unwrap();
    let messages: Vec<Value> = std::str::from_utf8(&output)
        ...
        .map(|line| serde_json::from_str(line).unwrap())
        .collect();
    let recorded_ack = messages
        .iter()
        .find(|message| message["gcp_command"] == wire::ACK)
        .unwrap();
    assert_eq!(recorded_ack["status"], "decoded", "{recorded_ack}");
    assert_eq!(recorded_ack["key_hex"], hex::encode([7; 16]));
    assert_eq!(recorded_ack["key_direction"], "download");
    assert_eq!(recorded_ack["key_offset"], 0);
    assert_eq!(recorded_ack["source_event"], 2);
    assert_eq!(recorded_ack["completed_event"], 4);
    let subsequent = messages
        .iter()
        .find(|message| message["gcp_command"] == wire::DATA)
        .unwrap();
    assert_eq!(subsequent["failure_code"], "missing_key");
    assert_eq!(subsequent["key_direction"], "upload");
    assert_eq!(subsequent["key_offset"], syn.len());
}
