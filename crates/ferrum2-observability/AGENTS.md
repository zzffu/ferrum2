...
The repository-level `AGENTS.md` remains in force. This crate is the zero-Ferrum-dependency telemetry boundary. Its crate root is a curated façade: `trace/{schema,subscriber,emit}` owns approved trace vocabulary and caller-owned JSON subscribers, while `metrics/{family,core,tun,rules_dns,render}` owns isolated registries and deterministic OpenMetrics text behind one public `Metrics` façade. It does not install global tracing state, start listeners, or spawn tasks; endpoint serving and process composition belong elsewhere.
