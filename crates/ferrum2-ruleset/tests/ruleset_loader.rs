...
fn source() -> ferrum2_ruleset::RuleSetRemoteSource {
...
async fn four_pinned_binary_rule_sets_load_into_one_publishable_snapshot() {
    ...
    let materialized = materialize_rule_sets(&loader, sources, 17)
        .await
        .expect("four RuleSet snapshot");
    assert_eq!(materialized.registry().generation(), 17);
    ...
    let snapshot = materialized.registry().snapshot();
...
async fn initial_snapshot_retains_each_degraded_cache_failure() {
    ...
    let materialized = materialize_rule_sets(&loader, vec![source()], 4)
        .await
...
async fn complete_snapshot_refreshes_atomically_and_failures_keep_generation() {
    ...
    assert_eq!(materialized.registry().generation(), 1);
    assert_eq!(materialized.rule_set_ids().len(), 1);
    assert_eq!(
        materialized.dispositions(),
