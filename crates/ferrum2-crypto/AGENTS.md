...
Secret owners must remain non-cloneable, redacted, explicitly clearable where exposed, and zeroized on drop. Preserve profile-specific 16/32-byte widths, zero-based u96 little-endian TCP nonces, authenticated-open behavior, and state commits only after successful operations. `TcpOpener::open_in_place` is destructive: authentication failure clears the ciphertext body while leaving the nonce uncommitted and the trailing tag in place. UDP final-wire reservations own method layout and packet-counter commit; dropping or failing a reservation clears its complete wire range. UDP owned-wire open is destructive, clears every candidate-plaintext byte on authentication failure, and returns the authenticated semantic body as a range without moving it. Nonce or packet-ID exhaustion must fail closed without mutating buffers or counters. `SecureRandom` failures have no fallback; response salts and UDP session IDs use bounded full-width collision retries. Do not expose keys, salts, nonces, identities, primitive source errors, or unauthenticated plaintext through diagnostics or partial results.
