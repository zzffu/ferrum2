...
fn poll_payload_once<S: TransportIo>(
    ...
    match Pin::new(&mut *io).poll_read_buf(cx, scratch, wire_len - filled) {
        Poll::Pending => {
            ...
            DataPoll::Pending(DataRx::Payload { wire_len, filled })
        }
        Poll::Ready(Err(_)) => {
...
pub(super) fn poll_data_fill<S: TransportIo>(
    ...
    // A completed length polls its payload once. If that transition observes
    // Pending, it requests exactly one scheduler retry. Later payload Pending
    // results rely only on the transport waker; partial ready reads still yield.
    match state {
            ...
            match Pin::new(&mut *io).poll_read_buf(cx, scratch, remaining) {
                ...
                Poll::Ready(Ok(read)) => {
                    ...
                    if filled < ENCRYPTED_LENGTH_LEN {
                        ...
                        poll_payload_once(
                            io, opener, scratch, wire_len, 0, true, lifecycle, observer, cx,
                        )
                    }
                }
            }
        ...
        DataRx::Payload { wire_len, filled } => poll_payload_once(
            io, opener, scratch, wire_len, filled, false, lifecycle, observer, cx,
        ),
