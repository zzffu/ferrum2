...
#[tokio::test]
async fn request_fixed_region_is_single_operation_then_variable_accepts_one_byte_fragments() {
    let keys = provider();
    ...
    reads.extend(
        request[REQUEST_FIRST_READ_LEN..]
            .iter()
            .map(|byte| vec![*byte]),
    );
    ...
    assert_eq!(
        observed.read_calls,
        1 + request.len() - REQUEST_FIRST_READ_LEN
    );
}

#[tokio::test]
async fn response_fixed_region_is_single_operation_then_payload_accepts_one_byte_fragments() {
    let keys = provider();
    let clock = FakeClock::new(NOW, 0);
