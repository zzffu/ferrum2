...
#[tokio::test]
async fn ready_length_pending_payload_adds_one_synthetic_wake_only_on_transition() {
    let keys = provider();
    let clock = FakeClock::new(NOW, 0);
    let random = ScriptedRandom::new([]);
    let replay = TcpReplayStore::new(1024).expect("capacity");
    ...
    let mut flow = inbound
        .accept_stream(io.with_pending_reads_after(3, 2))
        .await
        .expect("request")
        .stream;
    let wake_counter = Arc::new(WakeCounter(AtomicUsize::new(0)));
    ...
    assert_eq!(
        ...
        2,
        "the transition adds one synthetic wake to the transport wake"
    );
    ...
    assert_eq!(
        observation.lock().expect("observation").read_calls,
        baseline + 1,
        "a later pending payload still completes no transport read"
    );
    assert_eq!(
        wake_counter.0.load(Ordering::SeqCst),
        3,
        "the established payload state relies only on the transport wake"
    );
