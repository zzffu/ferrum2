...
use ferrum2_shadowsocks::{
    DetectionReason, REQUEST_FIRST_READ_LEN, ShadowsocksError, TcpReplayStore,
    accept_server_request,
};

use common::{
    FakeClock, NOW, RecordingIo, custom_request_wire, provider, salt_from_u64, valid_request_wire,
};
...
async fn auth_truncation_at_every_initial_chunk_prefix_is_rejected() {
    ...
    for truncated in 0..REQUEST_FIRST_READ_LEN {
        ...
        assert!(matches!(
            accept_server_request(io, &keys, &clock, &replay).await,
