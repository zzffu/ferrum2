...
struct PartialWritePlain {
    accepted: Arc<Mutex<Vec<u8>>>,
    write_calls: usize,
    first_write_limit: usize,
}

impl AsyncRead for PartialWritePlain {
    fn poll_read(
        self: Pin<&mut Self>,
        _cx: &mut Context<'_>,
...
impl AsyncWrite for PartialWritePlain {
    fn poll_write(
        mut self: Pin<&mut Self>,
        _cx: &mut Context<'_>,
        source: &[u8],
    ) -> Poll<io::Result<usize>> {
        let limit = if self.write_calls == 0 {
            self.first_write_limit
        } else {
            source.len()
        };
        self.write_calls += 1;
        let written = source.len().min(limit);
        self.accepted
            .lock()
...
#[tokio::test]
async fn completed_carried_download_polls_one_next_read_without_synthetic_wake() {
    let keys = provider();
    let clock = FakeClock::new(NOW, 0);
    ...
    let pending_waker = Arc::new(Mutex::new(None));
    let io = inner.with_silent_pending_read_after(4, Arc::clone(&pending_waker));
    let inbound = ShadowsocksTcpInbound::new(&keys, &clock, &random, &replay);
    let mut flow = inbound
        .accept_stream(io)
        .await
    ...
    let mut plain = PartialWritePlain {
    ...
    assert_eq!(wake_counter.0.load(Ordering::SeqCst), 2);
    assert_eq!(&*accepted.lock().expect("accepted plaintext"), &first[..3]);

    assert!(matches!(relay.as_mut().poll(&mut cx), Poll::Pending));
    assert_eq!(
        observation.lock().expect("observation").read_calls,
        4,
        "the carried plaintext completes before exactly one pending tunnel read"
    );
    assert_eq!(
        wake_counter.0.load(Ordering::SeqCst),
        2,
        "carried completion does not manufacture a wake for a pending tunnel read"
    );
    ...
    let child_waker = pending_waker
        ...
        .expect("pending waker")
        .take()
        .expect("tunnel read registered the relay waker");
    child_waker.wake();
    assert_eq!(wake_counter.0.load(Ordering::SeqCst), 3);

    assert!(matches!(relay.as_mut().poll(&mut cx), Poll::Pending));
    assert_eq!(observation.lock().expect("observation").read_calls, 5);
    ...
    let expected = first
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        *accepted.lock().expect("accepted plaintext"),
        expected,
        "partial carried writes neither repeat nor omit plaintext"
    );
    assert_eq!(
        observation.lock().expect("observation").read_calls,
        6,
        "each successful lower read was issued once"
    );
}
