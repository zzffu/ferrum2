...
/// Reproducible release-only micro workload. It intentionally reports timing
/// without asserting a machine-sensitive duration threshold:
///
/// `cargo test --release -p ferrum2-shadowsocks --features tokio --test
/// tcp_decrypt_timing --locked release_timing_for_steady_tcp_frame_decrypt --
/// --ignored --exact --nocapture --test-threads=1`
#[tokio::test(flavor = "current_thread")]
#[ignore = "release-only performance diagnostic"]
async fn release_timing_for_steady_tcp_frame_decrypt() {
...
fn print_sample(payload_len: usize, sample: usize, elapsed: Duration, checksum: u64) {
    println!(
        "tcp_decrypt_timing payload_bytes={payload_len} sample={sample} frames={MEASURED_FRAMES} \
...
fn print_summary(payload_len: usize, median: Duration) {
    println!(
        "tcp_decrypt_timing_summary payload_bytes={payload_len} samples={SAMPLES} \
