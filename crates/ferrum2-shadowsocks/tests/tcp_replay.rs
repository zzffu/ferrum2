...
use ferrum2_shadowsocks::{
    DetectionReason, ShadowsocksError, TcpReplayStore, accept_server_request,
};
use tokio::sync::Barrier;

use common::{FakeClock, NOW, RecordingIo, provider, salt_from_u64, valid_request_wire};
...
async fn timestamp_boundaries_are_inclusive() {
    ...
    .enumerate()
    {
        ...
        let result = accept_server_request(io, &keys, &clock, &replay).await;
        if accepted {
...
async fn exact_invalid_does_not_poison_then_duplicate_is_rejected() {
    ...
    assert!(matches!(
        accept_server_request(invalid_io, &keys, &clock, &replay).await,
    ...
    assert!(
        accept_server_request(first_io, &keys, &clock, &replay)
            .await
            .is_ok()
    );
    let (duplicate_io, observation) = RecordingIo::request(&wire);
    assert!(matches!(
        accept_server_request(duplicate_io, &keys, &clock, &replay).await,
    ...
    for round in 0..20_u64 {
        ...
        for _ in 0..64 {
            ...
            tasks.push(tokio::spawn(async move {
                ...
                barrier.wait().await;
                accept_server_request(io, keys.as_ref(), clock.as_ref(), replay.as_ref()).await
            }));
        }
        let mut accepted = 0;
        let mut replayed = 0;
...
async fn retention_uses_monotonic_59999_and_60000_boundaries_despite_wall_rollback() {
    ...
    assert!(matches!(
        accept_server_request(still_live_io, &keys, &clock, &replay).await,
        Err(ShadowsocksError::Detection(DetectionReason::Replay))
    ));
    ...
    assert!(
        accept_server_request(expired_io, &keys, &clock, &replay)
            .await
            .is_ok()
    );
    assert_eq!(replay.entry_count().expect("snapshot"), 1);
...
async fn capacity_full_fails_closed_without_live_eviction() {
    ...
    for value in 0..1024_u64 {
        ...
        let (io, _) = RecordingIo::request(&wire);
        assert!(
            accept_server_request(io, &keys, &clock, &replay)
                .await
                .is_ok(),
            "entry {value}"
        );
    ...
    assert!(matches!(
        accept_server_request(new_io, &keys, &clock, &replay).await,
    ...
    assert!(matches!(
        accept_server_request(old_io, &keys, &clock, &replay).await,
        Err(ShadowsocksError::Detection(DetectionReason::Replay))
    ));
    assert_eq!(replay.entry_count().expect("snapshot"), 1024);
}
