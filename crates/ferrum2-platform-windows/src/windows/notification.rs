...
pub(super) fn cancel_notification_handles<T, C>(
    handles: &mut Vec<T>,
...
pub(super) fn leak_notification_owners<T, C>(handles: &mut Vec<T>, context: &mut Option<C>) {
    std::mem::forget(std::mem::take(handles));
    std::mem::forget(context.take());
}

pub(super) fn close_notification_handles<T, C>(
    handles: &mut Vec<T>,
    context: &mut Option<C>,
...
pub(super) fn subscribe_notification_sequence<H, C>(
    context: C,
...
pub(super) trait NetworkChangeWaitOperations {
...
pub(super) fn wait_for_network_change(
