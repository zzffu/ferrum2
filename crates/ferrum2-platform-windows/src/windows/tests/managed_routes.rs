use super::support::{
    ...
    delete_managed_address, delete_managed_route, increment_unicast_interface_luid,
    initialize_managed_address, install_managed_routes, ipv4_sockaddr, ipv6_sockaddr,
    managed_address_matches, take_last_owned_route,
};
...
fn managed_address_readback_and_cleanup_are_exact_and_foreign_safe() {
    ...
    let changed = [
        {
            let mut row = actual;
            increment_unicast_interface_luid(&mut row);
            row
        },
        {
            let mut row = actual;
