use super::support::{
    DNS_SETTING_IPV6, DNS_SETTING_NAMESERVER, DnsFamily, Error, InterfaceCandidate,
    InterfaceIdentity, IpPrefix, Ipv4DnsSettings, Ipv4Prefix, Ipv6DnsSettings, ManagedDnsLease,
    ManagedDnsOperations, RouteFingerprint, copy_bounded_wide, dns_settings_query_flags,
    eligible_interface_identity, install_managed_dns, ipv4_dns_settings_input,
    ipv6_dns_settings_input, managed_dns_matches, normalize_dns_settings, prepare_managed_intent,
    restore_managed_dns,
};
...
fn managed_dns_snapshots_reads_back_and_conditionally_restores() {
    ...
    assert_eq!(copy_bounded_wide(None).unwrap(), None);
    let empty = [0_u16];
    assert_eq!(copy_bounded_wide(Some(&empty)).unwrap(), None);
    let value = [
        b'1' as u16,
        b'.' as u16,
        b'1' as u16,
    ...
    assert_eq!(
        copy_bounded_wide(Some(&value)).unwrap().as_deref(),
        Some(&value[..5])
    );
    let unterminated = vec![1_u16; 4097];
    assert!(copy_bounded_wide(Some(&unterminated)).is_err());
}
