...
use super::super::core::strict_route::{
    StrictRouteOperations, StrictRouteSession, guid_matches, wfp_readback_present,
};
use super::loader::{current_executable, wide};
use crate::Error;
use crate::strict_route::{
    MAX_WFP_APP_ID_BYTES, StrictRouteCondition, StrictRouteRule, StrictRouteRuleKind,
};
...
/// # Safety
///
/// `raw` must be null or point to a readable NUL-terminated UTF-16 allocation that remains alive
/// for this call. Comparison short-circuits at the first differing unit and never reads beyond the
/// expected string's terminator.
pub(super) unsafe fn raw_wide_matches(raw: *const u16, expected: &str) -> bool {
    if raw.is_null() {
        return false;
    }
...
/// # Safety
///
/// Every pointer selected by the tagged unions in `raw` must refer to the live allocation returned
/// by WFP and remain readable for the sizes recorded in the row.
pub(super) unsafe fn raw_strict_route_condition_matches(
...
/// # Safety
///
/// All pointer-bearing fields in `raw` and its conditions must belong to the live allocation
/// returned by `FwpmFilterGetById0` and remain valid for the duration of this call.
pub(super) unsafe fn raw_strict_route_filter_matches(
    id: u64,
    raw: &FWPM_FILTER0,
    ...
    expected.conditions.iter().all(|expected| {
        raw_conditions
            .iter()
            .any(|raw| unsafe { raw_strict_route_condition_matches(raw, expected) })
    })
}
...
impl StrictRouteOperations for PlatformStrictRouteOperations {
    ...
    fn filter_matches(
        ...
        // SAFETY: `allocation` owns the complete FwpmFilterGetById0 result through this readback,
        // including its display name, condition array, and tagged-union pointees.
        Ok(unsafe { raw_strict_route_filter_matches(id, allocation.get()?, rule) })
    }
