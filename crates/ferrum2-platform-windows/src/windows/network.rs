...
use ferrum2_net::{
    NetworkInterfaceCatalog as SharedNetworkInterfaceCatalog, NetworkInterfaceCatalogError,
    NetworkInterfaceObservation, SystemBestRoute,
};
...
pub(super) struct RouteFingerprint {
    pub(super) interface_luid: u64,
    pub(super) interface_index: u32,
    pub(super) destination: std::net::IpAddr,
    pub(super) prefix_length: u8,
    pub(super) next_hop: std::net::IpAddr,
...
impl WindowsNetworkInterfaceCatalog {
    ...
    pub(super) fn managed_tun(&self) -> Result<Option<InterfaceIdentity>, Error> {
        self.managed_tun
            .lock()
            .map(|value| *value)
            .map_err(|_| Error)
    ...
    pub(super) fn set_managed_tun(&self, identity: InterfaceIdentity) -> Result<(), Error> {
    ...
    pub(super) fn clear_managed_tun(&self, identity: InterfaceIdentity) -> Result<(), Error> {
...
impl SharedNetworkInterfaceCatalog for WindowsNetworkInterfaceCatalog {
    fn read_interfaces(
        &self,
    ) -> Result<Vec<NetworkInterfaceObservation>, NetworkInterfaceCatalogError> {
        super::ffi::network::read_network_interface_observations(
            self.managed_tun()
    ...
    fn system_best_route(
        ...
        super::ffi::network::system_best_route(
            destination,
            self.managed_tun()
                .map_err(|_| NetworkInterfaceCatalogError)?,
        )
...
pub struct UnderlayPolicy {
    pub(super) fixed: Arc<[(std::net::SocketAddr, RouteFingerprint)]>,
    pub(super) target_binder: bool,
    pub(super) valid: Arc<AtomicBool>,
    pub(super) generation: Arc<AtomicU64>,
    pub(super) accepted_generation: Arc<AtomicU64>,
    pub(super) owned_luid: Arc<AtomicU64>,
    pub(super) owned_index: Arc<AtomicU32>,
}
...
impl UnderlayPolicy {
    pub fn bind_fixed<T: AsRawSocket>(
        ...
        super::ffi::network::bind_fixed_socket(self, socket, endpoint)
    }
    ...
    pub fn bind_target<T: AsRawSocket>(
        ...
        super::ffi::network::bind_target_socket(self, socket, target)
    }
    ...
    pub(super) fn begin_binding(&self) -> Result<u64, Error> {
        let generation = self.generation.load(Ordering::Acquire);
        self.require_generation(generation)?;
        Ok(generation)
    }

    pub(super) fn require_generation(&self, generation: u64) -> Result<(), Error> {
        (self.valid.load(Ordering::Acquire)
            && self.accepted_generation.load(Ordering::Acquire) == generation
    ...
    pub(super) fn accept_generation(&self, generation: u64) {
        self.accepted_generation
            .store(generation, Ordering::Release);
    }

    pub(super) fn set_owned_identity(&self, owned: InterfaceIdentity) -> Result<(), Error> {
    ...
    pub(super) fn owned_identity(&self) -> Result<InterfaceIdentity, Error> {
...
pub(super) trait UnderlayOperations {
...
pub(super) const fn same_ip_family(left: std::net::IpAddr, right: std::net::IpAddr) -> bool {
    matches!(
        (left, right),
...
#[cfg(test)]
pub(super) fn snapshot_underlay_with(
...
pub(super) fn snapshot_underlay_at(
    config: &ManagedNetworkConfig,
...
pub(super) fn refresh_underlay_with(
    config: &ManagedNetworkConfig,
...
pub(super) fn classify_underlay_refresh<T>(result: Result<T, Error>) -> Result<T, Error> {
    result.map_err(|_| Error::recoverable_session())
}

pub(super) fn underlay_matches_with(
    policy: &UnderlayPolicy,
    owned: InterfaceIdentity,
    operations: &mut impl UnderlayOperations,
) -> Result<bool, Error> {
...
#[cfg(test)]
pub(super) fn underlay_snapshot_matches(
    policy: &UnderlayPolicy,
    owned: InterfaceIdentity,
    expected_generation: u64,
