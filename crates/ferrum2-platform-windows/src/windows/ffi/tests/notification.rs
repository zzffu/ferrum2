use super::support::{
    AF_UNSPEC, Error, ErrorKind, MIB_IPFORWARD_ROW2, MIB_IPINTERFACE_ROW, MIB_UNICASTIPADDRESS_ROW,
    NET_LUID_LH, NetworkChangeWaitOperations, NetworkChangeWaitOutcome, NotificationContext,
    NotificationOwners, address_changed, cancel_notification_handles, classify_notification_luid,
    close_notification_handles, interface_changed, leak_notification_owners,
    managed_notification_family, route_changed, subscribe_notification_sequence,
...
fn network_change_notifications_cover_each_callback_and_runtime_owned_events() {
    ...
    for callback in [Callback::Route, Callback::Interface, Callback::Address] {
        for (name, actions, changed) in cases {
            ...
            let context =
                (notifications.context.as_deref().unwrap() as *const NotificationContext).cast();
