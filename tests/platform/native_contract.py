"""Portable native behavior contract for ferrum2 release binaries."""
...
def binary_specs(
    *, root: Path, target: str, client: Path, server: Path
) -> tuple[BinarySpec, BinarySpec]:
    client = client.resolve()
    server = server.resolve()
    require(
        client == expected_artifact(root, target, "ferrum2-client"),
        "unexpected-client-artifact-path",
    )
    require(
        server == expected_artifact(root, target, "ferrum2-server"),
...
def assert_native_contract(specs: tuple[BinarySpec, BinarySpec]) -> None:
