    ...
    def test_startup_failure_preserves_every_started_product_log(self) -> None:
        for family in ("IPv4", "IPv6"):
            for failed_role, expected_roles in (("client", ["client"]), ("server", ["client", "server"])):
                with self.subTest(failed_role=failed_role, address_family=family):
                    self.check_failure(failed_role, expected_roles, family=family)

    def test_export_failure_does_not_replace_the_startup_error(self) -> None:
        for family in ("IPv4", "IPv6"):
            with self.subTest(address_family=family):
                self.check_failure("server", [], export_failure=True, family=family)

    def check_failure(self, failed_role: str, expected_roles: list[str], *, export_failure: bool = False, family: str = "IPv4") -> None:
        with tempfile.TemporaryDirectory(prefix="ferrum2-startup-evidence-") as directory:
            ...
            script.write_text(
                r"""
param([string]$Owners, [string]$FixtureRoot, [string]$FailedRole, [string]$ExportFailure, [string]$AddressFamily)
...
function Wait-Ferrum2Metric {
    param($Process, $Port, $Name, $Minimum, $AddressFamily)
...
""",
            ...
            completed = subprocess.run(
                ["pwsh", "-NoProfile", "-File", str(script), "-Owners", str(OWNERS),
                 ...
                 "-ExportFailure", str(export_failure).lower(), "-AddressFamily", family],
