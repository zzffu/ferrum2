...
`-PlanOnly`, `-RecoveryOnly`, and the acknowledged real run. Plan/run select `-AddressFamily
IPv4|IPv6` (default IPv4); recovery derives the family from verified ledger identities. Do not add
profiles, suites, repeat counts, guest stages, or alternate runners.
...
may touch only ledger-owned Wintun, RFC 2544 `/32` or run-unique ULA `/128` explicit routes/addresses,
processes, ports, and the product's dynamic strict-route and exact TCP-ingress WFP identities.
Only the owned TUN may have its connected IPv4 `/30` or IPv6 `/126` prefix. Standing authorization
for this development host is recorded in `docs/windows-tun-qualification.md`; do not remove the
per-run acknowledgement or privilege gates. The acknowledged run also owns narrowly
...
requested candidate, bundle and address-family identities, less than 900 seconds, and zero cleanup residue.
