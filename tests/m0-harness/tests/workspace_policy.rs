...
fn tun_dependency_edges_and_unsafe_exception_are_exact() {
    ...
    let selector_contract = [
        ...
        "if ($dispatchTarget -eq \"windows-tun-full\") {",
        "$transportOutput = & pwsh -NoProfile -File tests/platform/qualify_windows_tun.ps1 -Mode udp -RunToken $env:FERRUM2_TUN_RUN_TOKEN",
        "if ($LASTEXITCODE -ne 0) { throw \"udp controller failed\" }",
        "$cyclesOutput = & pwsh -NoProfile -File tests/platform/qualify_windows_tun.ps1 -Mode cycles -RunToken $env:FERRUM2_TUN_RUN_TOKEN",
        "if ($LASTEXITCODE -ne 0) { throw \"cycles controller failed\" }",
        "$transportMarker = \"m15_windows_tun_e2e status=PASS profile=transport functional=16/16 cleanup=PASS sha=$env:GITHUB_SHA run_id=$env:GITHUB_RUN_ID run_attempt=$env:GITHUB_RUN_ATTEMPT\"",
        "$cyclesMarker = \"m15_windows_tun_cycles status=PASS cycles=100/100 cleanup=PASS sha=$env:GITHUB_SHA run_id=$env:GITHUB_RUN_ID run_attempt=$env:GITHUB_RUN_ATTEMPT\"",
        "@($transportOutput | Where-Object { $_ -ceq $transportMarker }).Count -ne 1",
        "@($cyclesOutput | Where-Object { $_ -ceq $cyclesMarker }).Count -ne 1",
        "$output = @($expectedMarker)",
