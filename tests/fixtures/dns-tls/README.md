...
These files are shared by `ferrum2-dns` encrypted-transport tests and `ferrum2-ruleset` HTTPS
transport tests. They are synthetic test-only artifacts generated locally with
...
The artifacts are repository-authored test data under the project's `GPL-3.0-only` license. Their
checked-in bytes are immutable shared fixtures: a refresh must reproduce the credentials, preserve
the `resolver.test` identity, and update every consumer and digest in one provenance change.
