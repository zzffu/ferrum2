...
$failure = $null
try { New-Ferrum2ProductPorts -Context @{} -Sequence 1 -Topology EndToEnd | Out-Null }
...
function Write-Ferrum2TrialConfigs {
    param($Context, $Network, $Loopback, $AdapterName, $Topology, $ServerPort,
...
function Start-Ferrum2OwnedNativeProcess {
    ...
    if ($LogPrefix -ceq 'trial-1-server') {
...
$failure = $null
try {
    Start-Ferrum2ProductTrial -Context $context -Sequence 1 -Topology EndToEnd -Loopback @{} `
        -Member @{ client = "$Root/client.exe"; server = "$Root/server.exe" } `
        -Network @{ adapter_name_prefix = 'fixture'; support_address = '198.18.0.1' } | Out-Null
} catch { $failure = $_.Exception.Message }
...
''')
    ...
    def test_joined_native_readback_drives_reservation_and_records_ranges(self) -> None:
        self.run_script(r'''
Add-Type -Path (Join-Path $Owners 'PerformanceProcessOwner.cs')
...
''')
    ...
    def test_early_process_exit_does_not_wait_for_metrics_timeout(self) -> None:
        self.run_script(r'''
Add-Type -TypeDefinition 'public static class Ferrum2PerfProcessGroup {
    public static bool Wait(uint pid, uint ms) { return true; }
    public static int ExitCode(uint pid) { return 1; }
}'
function Get-Ferrum2Metrics { throw 'metrics must not be polled after process exit' }
$failure = $null
try { Wait-Ferrum2Metric -Process @{ pid = 42 } -Port 1234 -Name ferrum2_network_generation -Minimum 1 }
catch { $failure = $_.Exception.Message }
if ($failure -cne 'product exited before metric readiness: ferrum2_network_generation; exit=1') {
    throw "wrong readiness failure: $failure"
}
''')
