...
class WindowsTunPerformanceTests(unittest.TestCase):
    ...
    def udp_diagnostic_evidence(
        ...
        *,
        ...
        identity = {
            ...
            "server_sha256": "6" * 64,
            "harness_sha256": "7" * 64,
            "runner_sha256": CONTROL.WINDOWS_TUN_RUNNER_SOURCE_SHA256,
        ...
        workload_schema = CONTROL.WINDOWS_TUN_UDP_WORKLOAD_LEDGER_SCHEMA
        support_schema = CONTROL.WINDOWS_TUN_UDP_SUPPORT_LEDGER_SCHEMA
        workload_event = {
        ...
        for name in CONTROL.WINDOWS_TUN_UDP_CAPTURE_FILES:
        ...
        capture_manifest = {
            ...
            "stop_status": "PASS",
            "expected_files": list(CONTROL.WINDOWS_TUN_UDP_CAPTURE_FILES),
            "files": [
                {
                    "file": name,
        ...
        observations = {
            stage: "UNKNOWN" for stage in CONTROL.WINDOWS_TUN_UDP_OBSERVATION_STAGES
        }
        ...
        failure_summary = {
            "schema": CONTROL.WINDOWS_TUN_UDP_FAILURE_SUMMARY_SCHEMA,
            "qualification": False,
            "run_nonce": run_nonce,
        ...
        row = {
            "schema": CONTROL.WINDOWS_TUN_UDP_DIAGNOSTIC_SCHEMA,
            "qualification": False,
    ...
    def validate_udp_diagnostic(
        ...
        return CONTROL.validate_windows_tun_udp_diagnostic(
            plan=plan,
            plan_sha256=plan_sha256,
            evidence_root=root,
            parent_sha=self.AA_SHA,
    ...
    def test_udp_diagnostic_is_explicitly_rejected_by_formal_paths(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "cannot be validated as a formal"
            ):
                CONTROL.validate_windows_tun_trial(
                    row,
                    plan=plan,
            ...
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "cannot enter the formal.*reducer"
            ):
                CONTROL.summarize_windows_tun_evidence(
                    plan=plan,
                    evidence_root=root,
    ...
    def test_udp_diagnostic_rejects_zero_byte_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.write_udp_diagnostic_document(root, row)
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "positive u64"
            ):
                self.validate_udp_diagnostic(root, plan, plan_sha256)
    ...
    def test_udp_diagnostic_support_packet_boundary_is_ledger_bound(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            support_rx = {
                "schema": CONTROL.WINDOWS_TUN_UDP_SUPPORT_LEDGER_SCHEMA,
                "record_type": "event",
                "event_index": 0,
                "timestamp_qpc": 2_000,
                "timestamp_qpc_frequency": 1_000_000_000,
            ...
            self.write_udp_diagnostic_document(root, row)
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "must be a valid port"
            ):
                self.validate_udp_diagnostic(root, plan, plan_sha256)
            ...
            self.write_udp_diagnostic_document(root, row)
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "TX is not ordered after"
            ):
                self.validate_udp_diagnostic(root, plan, plan_sha256)
    ...
    def test_udp_diagnostic_capture_manifest_and_cleanup_are_bound(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                CONTROL.CandidateControlError, "capture file binding"
            ):
                self.validate_udp_diagnostic(root, plan, plan_sha256)
    ...
    def test_udp_diagnostic_endpoints_paths_and_line_bounds_are_closed(self) -> None:
        ...
        for json_bytes, accepted in ((4_096, True), (4_097, False)):
            with self.subTest(json_bytes=json_bytes), tempfile.TemporaryDirectory() as directory:
                ...
                if accepted:
                    ...
                    with self.assertRaisesRegex(
                        CONTROL.CandidateControlError, "line exceeds"
                    ):
                        self.validate_udp_diagnostic(root, plan, plan_sha256)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.write_udp_diagnostic_document(root, row)
            with self.assertRaisesRegex(CONTROL.CandidateControlError, "unterminated"):
                self.validate_udp_diagnostic(root, plan, plan_sha256)
    ...
    def test_udp_diagnostic_requires_reviewed_aa_sequence_and_plan_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = pathlib.Path(directory)
            plan, row, plan_sha256 = self.udp_diagnostic_evidence(root)
            comparison = CONTROL.create_windows_tun_plan(
                run_kind="comparison", decision_policy=self.policy()
            )
            with self.assertRaisesRegex(CONTROL.CandidateControlError, "calibration-aa A/A"):
                CONTROL.validate_windows_tun_udp_diagnostic(
                    plan=comparison,
                    plan_sha256=plan_sha256,
                    evidence_root=root,
                    parent_sha=self.AA_SHA,
    ...
    def test_udp_diagnostic_rejects_duplicate_nonfinite_and_oversize_json(self) -> None:
        cases = (
            b'{"schema":"x","schema":"y"}',
            b'{"schema":NaN}',
            b"{" + b" " * CONTROL.WINDOWS_TUN_UDP_DIAGNOSTIC_MAX_BYTES + b"}",
        )
        for raw in cases:
