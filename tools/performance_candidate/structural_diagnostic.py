...
def validate_structural_diagnostic(
    ...
    *,
    ...
    if _required_u64(row, "schema_version", positive=True) != STRUCTURAL_SCHEMA_VERSION:
        raise CandidateControlError("structural diagnostic schema version is unsupported")
    _required_string(row, "kind", expected=STRUCTURAL_KIND)
    _required_string(row, "scenario", expected=STRUCTURAL_SCENARIO)
    ...
    _validate_counter_schema(row["counter_schema"])
    snapshots = _validate_snapshots(row["snapshots"])
    _validate_overflow(row["overflow"])
    computed = _compute_deltas(snapshots)
    _validate_recorded_deltas(row["deltas"], computed)
...
def _validate_counter_schema(value: object) -> None:
    ...
    if len(value) != 49:
        raise CandidateControlError("structural counter_schema must contain 49 families")
...
def _validate_snapshots(value: object) -> dict[str, dict[str, dict[str, int]]]:
    if type(value) is not dict:
    ...
    for endpoint in ("client", "server"):
        ...
        snapshots[endpoint] = {
            phase: _counter_values(endpoint_value[phase], f"{endpoint} {phase}")
            for phase in ("before", "after")
        }
    return snapshots


def _counter_values(value: object, name: str) -> dict[str, int]:
    if type(value) is not dict:
...
def _validate_recorded_deltas(
    value: object, computed: dict[str, dict[str, int]]
) -> None:
    if type(value) is not dict:
    ...
    for aggregation in ("client", "server", "merged"):
        recorded = _counter_values(value[aggregation], f"{aggregation} deltas")
