...
.DESCRIPTION
This probe is intentionally run before the managed Ferrum2 TUN adapter exists. It validates the
...
if (Get-NetAdapter -Name $ManagedAdapterName -IncludeHidden -ErrorAction SilentlyContinue) {
    throw "managed Ferrum2 TUN adapter must be absent during underlay selection"
}
