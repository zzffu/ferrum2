...
provisioner and an already provisioned manifest-bound support listener. The listener
...
$windowsTunRoot = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "..") `
...
$labRoot = Join-Path $windowsTunRoot "lab"
$guestNetworkPathProbePath = Join-Path $labRoot `
    "get_windows_tun_guest_network_path.ps1"
$hostNetworkPathHelperPath = Join-Path $labRoot `
    "windows_tun_host_network_path.ps1"
$topologyRuntimePath = Join-Path $labRoot `
...
$performanceSourceBundleBytes = [IO.File]::ReadAllBytes($performanceSourceBundlePath)
$performanceSourceBundle = $utf8Strict.GetString($performanceSourceBundleBytes) |
...
$bundleBootstrapSha256 = [Convert]::ToHexString(
    [Security.Cryptography.SHA256]::HashData($bundleBootstrapBytes)
).ToLowerInvariant()
if ($bundleBootstrapEntry.Count -ne 1 -or $bundleBootstrapBytes.LongLength -ne
        [long]$bundleBootstrapEntry[0].bytes -or
    $bundleBootstrapSha256 -cne [string]$bundleBootstrapEntry[0].sha256) {
...
$verifiedPerformanceSource = Read-Ferrum2BootstrapFlatSourceClosure `
    ...
    -ManifestBytes $performanceSourceBundleBytes `
...
$performanceSourceBundle = $verifiedPerformanceSource.Manifest
...
$provisioningManifestMember = Add-Ferrum2BootstrapSourceDependency `
    ...
    -RepositoryRoot $repositoryRoot -RequiredRoot $repositoryRoot `
    -RelativePath $provisioningManifestRelative
$provisioningManifest = $provisioningManifestMember.Text |
    ConvertFrom-Json -Depth 8 -ErrorAction Stop
foreach ($entry in @($provisioningManifest.files)) {
    Add-Ferrum2BootstrapSourceDependency -Closure $verifiedPerformanceSource `
        -RepositoryRoot $repositoryRoot -RequiredRoot $repositoryRoot `
        -RelativePath ([string]$entry.path) | Out-Null
}
$capturedSourceStage = Open-Ferrum2BootstrapLockedStage `
    -Closure $verifiedPerformanceSource `
    -ManifestRelativePath $performanceSourceBundleRelative `
    -NamePrefix 'ferrum2-performance-source'
$capturedRepositoryRoot = [string]$capturedSourceStage.Root
$capturedWindowsTunRoot = Join-Path $capturedRepositoryRoot 'tools\windows-tun'
$capturedLabRoot = Join-Path $capturedWindowsTunRoot 'lab'
$collectorPath = Join-Path $capturedWindowsTunRoot `
    'performance\collect_windows_tun_performance_trial.ps1'
$udpBoundaryCollectorPath = Join-Path $capturedWindowsTunRoot `
    'performance\collect_windows_tun_udp_boundary_diagnostic.ps1'
$guestNetworkPathProbePath = Join-Path $capturedLabRoot `
    'get_windows_tun_guest_network_path.ps1'
$hostNetworkPathHelperPath = Join-Path $capturedLabRoot `
    'windows_tun_host_network_path.ps1'
$topologyRuntimePath = Join-Path $capturedLabRoot `
    'windows_tun_hyperv_support_topology_runtime.ps1'
$performanceSourceBundlePath = Join-Path $capturedRepositoryRoot `
    $performanceSourceBundleRelative.Replace('/', [IO.Path]::DirectorySeparatorChar)
$labModulePath = Join-Path $capturedRepositoryRoot `
    "tools\powershell\Ferrum2.WindowsTun.Lab\Ferrum2.WindowsTun.Lab.psd1"
try {
Import-Module $labModulePath -Scope Local -Force -ErrorAction Stop
...
$performanceModuleRoot = Join-Path $capturedRepositoryRoot `
    "tools\powershell\Ferrum2.Performance"
...
exit $summaryExit
    ...
    Close-Ferrum2BootstrapLockedStage -Stage $capturedSourceStage
}
