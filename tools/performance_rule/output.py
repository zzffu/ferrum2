...
class OutputCleanupFailures(BaseExceptionGroup):
    """Failures from this owner's descriptor/temporary-file cleanup only."""
...
def write_encoded(encoded: str, output: Path) -> None:
    if len(encoded.encode("utf-8")) > EVIDENCE_MAX_BYTES:
        raise EvidenceLimit("controller evidence exceeds the encoded byte bound")
    ...
    if primary is not None:
        if cleanup_errors:
            raise primary from OutputCleanupFailures("output cleanup failed", cleanup_errors)
        raise primary
    if cleanup_errors:
        if len(cleanup_errors) == 1:
            raise cleanup_errors[0]
        raise OutputCleanupFailures("output cleanup failed", cleanup_errors)


def emit_result(result: dict[str, Any], output: Path | None) -> None:
    encoded = encode_result(result)
    if output is not None:
        write_encoded(encoded, output)
